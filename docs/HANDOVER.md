...
**Category:** addon
**Status:** v0.41.1, `/djbis` (and `/bis`), `Interface: 120100`. No remote. Built cards wait in `human-review/` for one trip to a live client. `0038` waits on a taint log from Rob. Every class: `0050` (builds) and `0051` (bars) are next.
_Last updated: 2026-09-24 (v0.41.1. Every class has a spec key, and gear stays druid-only behind `PlanTab.gearHere()`: card `0049`. Older entries: `docs/build/SESSION-LOG-ARCHIVE.md`.)_
...
Forty-three cards in `human-review/` are one trip to a live client; each lists its own looks. Type `/reload` first: the game folder holds v0.41.1 (deployed 2026-09-24). **Refresh the builds** when Dreamgrove updates a guide: `python update-builds.py --check`, then without `--check`.
