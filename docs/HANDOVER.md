...
**Category:** addon
**Status:** v0.33.3, `/djbis` (and `/bis`), `Interface: 120100`. No remote. `todo/` holds card `0035` only; `0031`, `0032` and `0034` passed a second review and sit in `human-review/`; `0033` was fixed a fourth time at v0.33.3 and is in `ai-review/` again; twenty-seven cards sit in `human-review/` and every one waits on one trip to a live client.
