...
**Category:** addon
**Status:** v0.48.11, `/dcp` (and `/djcp`), `Interface: 120100`. Remote: github.com/RobertLCraig/DjinnisClassProfiles (public). Built cards wait in `human-review/` for one trip to a live client.
...
The cards in `human-review/` are one trip to a live client; each lists its own looks. Type `/reload` first: the game folder holds v0.48.11 (deployed 2026-09-25). **Refresh the builds** when Dreamgrove updates a guide: `python update-builds.py --check`, then without `--check`.
