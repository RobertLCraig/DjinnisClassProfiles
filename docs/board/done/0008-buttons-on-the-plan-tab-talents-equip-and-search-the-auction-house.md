...
- 2026-09-22 Rob's screenshot at 0.17.4: the sash, the one piece in the bags, HAD gone on; the five left were all in the bank, and Equip only knew bags. So the bank press did work as built and the card's "not from the bank" fence was the whole gap. Fence dropped at 0.17.5: Equip and Equip all take from the bank too while it is open.
- 2026-09-22 Rob at 0.17.5: no Equip button after a reload, nor after moving the pieces to the bags. Cause: my 0.17.5 line `local _, _, where = state == "change" and PlanTab.holding(entry)`. In Lua an `and` keeps one value of a call, so `where` was always nil. Fixed at 0.17.6, and a new check draws the tab with a planned piece in a pretend bag and wants Equip and Equip all; it went red on the bug.
- 2026-09-22 Rob at 0.17.6: Equip works. A bought and socketed Flawless Masterful Garnet still read as wanted. Cause: two item ids carry that name, 240907 and 240908 (Raidbots gems.json); the sim asked for 240908 and the house sold the other. At 0.17.8 a gem matches by name when the client knows it, by id otherwise, in both the slot marks and the shopping list; the gem line also shows ids and what is in the socket, and SOCKET_INFO_CLOSE redraws the tab.
